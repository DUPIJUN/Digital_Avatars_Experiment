...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\32917\\OneDrive\\Desktop\\experiment_repos\\Experiment\\experiment_lastrun.py',
...
def setupWindow(expInfo=None, win=None):
    ...
    if win is None:
        ...
        win = visual.Window(
            ...
            monitor='testMonitor', color=(-0.4432, -0.4432, -0.4432), colorSpace='rgb',
        ...
        win.color = (-0.4432, -0.4432, -0.4432)
        win.colorSpace = 'rgb'
        win.backgroundImage = ''
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        ...
        font='Arial',
        units='pix', pos=(0, 0), draggable=False, height=50.0, wrapWidth=2000.0, ori=0.0, 
    ...
    prac_waiting_text = visual.TextStim(win=win, name='prac_waiting_text',
        text='',
        font='Arial',
        pos=(-250, 0), draggable=False, height=70.0, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-1.0);
    ...
    for thisPractice_loop in practice_loop:
        ...
        prac_movie = visual.MovieStim(
            win=win, name='prac_movie', units='height',
            noAudio=True,
        ...
        # 1. 获取参与者在帮助意愿量表上的得分
        # help_slider 是给量表起的名字
        prac_score = prac_help_slider.getRating()
        
        # 2. 根据得分区间，从Excel的列中选择对应的路径
        # 这些变量名（nodiff_path等）必须和 Excel 的表头完全一致
        if prac_score is not None:
            ...
            # 防止极端情况（没有点击就跳过了），默认给个不缓解
            prac_next_vid = NODIFFERENCE_PATH
        
        # 3. 把这个路径存入数据文件，方便以后进行分析
        ...
        prac_feedback = visual.MovieStim(
            win=win, name='prac_feedback', units='height',
            noAudio=True,
            filename=prac_next_vid,
            ori=0.0, pos=(0, 0), opacity=None,
            loop=False, anchor='center',
            size=(1.6, 1.0),
            depth=0.0,
    ...
    for thisTrial in trials:
        ...
        target_video = visual.MovieStim(
            win=win, name='target_video', units='height',
            noAudio=True,
            filename=ORIGINAL_PATH,
            ori=0.0, pos=(0, 0), opacity=None,
            loop=False, anchor='center',
        ...
        if score is not None:
            ...
            # 防止极端情况（没有点就跳过），默认给个不缓解
            next_vid = NODIFFERENCE_PATH
        
        # 3. 把这个路径存入数据文件，方便以后分析 [cite: 491]
        ...
        feedback_player = visual.MovieStim(
            win=win, name='feedback_player', units='height',
            noAudio=True,
            filename=next_vid,
            ori=0.0, pos=(0, 0), opacity=None,
            loop=False, anchor='center',
